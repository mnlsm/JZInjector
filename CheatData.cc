...
*/
    ...
    if (!found) {
        const auto& allWuPin = MiscData::GetAllWuPin();
...
std::vector<std::pair<std::wstring, int>> MiscData::GetWuGong(int pid, int code) {
    ...
    if (!found) {
        const auto& allWuGong = MiscData::GetAllWuGong();
...
const std::unordered_map<int, std::wstring>& MiscData::GetAllWuPin() {
...
const std::unordered_map<int, std::wstring>& MiscData::GetAllWuGong() {
    static std::unordered_map<int, std::wstring> s_datas;
    s_datas[1] = L"罗汉拳";
    s_datas[2] = L"逍遥游";
    s_datas[3] = L"五毒神掌";
