...
public:
   ...
   void AddListItem(LPCTSTR pstrText, int data)
   {
       ATLASSERT(!::IsBadStringPtr(pstrText, -1));
       CComBSTR bstr(pstrText);
       m_arrList.Add(bstr);
   ...
   int GetListItemData() {
       if (m_val.lVal < 0L || m_val.lVal >= m_arrData.GetSize()) return 0;
       return m_arrData[m_val.lVal];
   }
};
...
inline HPROPERTY PropCreateList(LPCTSTR pstrName, LPARAM lParam = 0)
{
    return new CPropertyListItem(pstrName, lParam);
}
