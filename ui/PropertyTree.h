...
{
public:
   typedef CWindowImplBaseT<TBase, CControlWinTraits> baseClass;
   DECLARE_WND_SUPERCLASS2(NULL, baseClass, TBase::GetWndClassName())

   enum { HORIZ_VALUE_GAP = 16 };
   ...
   {
      ATLASSERT(baseClass::m_hWnd==NULL);
      ATLASSERT(::IsWindow(hWnd));
   ...
   {
      ATLASSERT(::IsWindow(baseClass::m_hWnd));
   ...
   HTREEITEM InsertItem(HPROPERTY hProp, int nImage, int nSelectedImage, HTREEITEM hParent, HTREEITEM hInsertAfter = TVI_LAST)
   {
      ...
      if( hItem ) {
         hProp->SetOwner(baseClass::m_hWnd, (LPVOID) hItem);
         baseClass::Expand(hParent);
      }
      return hItem;
   }
   ...
   BOOL SetItemValue(HTREEITEM hItem, VARIANT* pValue)
   {
      ...
      if( m_iInplaceIndex && (baseClass::GetSelectedItem() == m_iInplaceIndex) ) _SpawnInplaceWindow(prop, m_iInplaceIndex);
      return bRes;
   }
   ...
   IProperty* GetItemProperty(HTREEITEM hItem) const {
       ATLASSERT(::IsWindow(baseClass::m_hWnd));
       IProperty* prop = reinterpret_cast<IProperty*>(TBase::GetItemData(hItem));
       ATLASSERT(prop);
       return prop;
   ...
   {
      ATLASSERT(::IsWindow(baseClass::m_hWnd));
      baseClass::ModifyStyle(TVS_CHECKBOXES | TVS_FULLROWSELECT, TVS_SHOWSELALWAYS);
      baseClass::SendMessage(WM_SETTINGCHANGE);
   }

   HTREEITEM _FindProperty(HPROPERTY prop, HTREEITEM hItem = NULL) const
   {
      if( hItem == NULL ) hItem = baseClass::GetRootItem();
      while( hItem != NULL ) {
         if( TBase::GetItemData(hItem) == (DWORD_PTR) prop ) return hItem;
         // Recurse into children
         HTREEITEM hChild = baseClass::GetChildItem(hItem);
         if( hChild != NULL ) {
            hChild = _FindProperty(prop, hChild);
            if( hChild && (TBase::GetItemData(hChild) == (DWORD_PTR) prop) ) return hChild;
         }
         // Get next sibling
         hItem = baseClass::GetNextSiblingItem(hItem);
      }
      return NULL;
   }
   ...
   void _GetInplaceRect(HTREEITEM iItem, RECT* pRect) const
   {
      ...
      ATLASSERT(pRect);
      RECT rcText = { 0 };
      baseClass::GetItemRect(iItem, &rcText, TRUE);
      RECT rcItem = { 0 };
      baseClass::GetItemRect(iItem, &rcItem, FALSE);
   ...
   void _DestroyInplaceWindow()
   {
      if( m_hwndInplace != NULL && ::IsWindow(m_hwndInplace) ) {
         ...
         if( ::GetFocus() != baseClass::m_hWnd && baseClass::IsChild(::GetFocus()) ) baseClass::SetFocus();
         // Destroy control
         switch( bKind ) {
   ...
   BOOL _SpawnInplaceWindow(IProperty* prop, HTREEITEM hItem)
   {
      ...
      if( hItem == NULL || hItem != baseClass::GetSelectedItem() ) return FALSE;
      if( !prop->IsEnabled() ) return FALSE;
      // Create a new editor window
      RECT rcValue = { 0 };
      _GetInplaceRect(hItem, &rcValue);
      ::InflateRect(&rcValue, 0, -1);
      m_hwndInplace = prop->CreateInplaceControl(baseClass::m_hWnd, rcValue);
   ...
   void _InvalidateItem(HTREEITEM hItem)
   {
      ...
      RECT rc = { 0 };
      baseClass::GetItemRect(hItem, &rc, FALSE);
      baseClass::InvalidateRect(&rc);
   }
   ...
   {
      LRESULT lRes = baseClass::DefWindowProc();
      _Init();
      return lRes;
   }
   ...
   LRESULT OnSetFocus(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
   {
      ...
      IProperty* prop = NULL;
      HTREEITEM hItem = baseClass::GetSelectedItem();
      if( hItem != NULL ) {
   ...
   LRESULT OnNavigate(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
   {
      ...
      case VK_UP:
      case VK_DOWN:
          baseClass::SelectItem(baseClass::GetNextItem(baseClass::GetSelectedItem(), wParam == VK_UP ? TVGN_PREVIOUSVISIBLE : TVGN_NEXTVISIBLE));
         break;
      }
      return 0;
   }
   ...
   LRESULT OnUpdateProperty(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
   {
      ...
      NMPROPERTYITEM nmh = { baseClass::m_hWnd, baseClass::GetDlgCtrlID(), PIN_ITEMCHANGING, prop };
      if( ::SendMessage(baseClass::GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh) == 0 ) {
         ...
         ::SendMessage(baseClass::GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh);
         // Repaint item
   ...
   LRESULT OnClick(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& bHandled)
   {
      ...
      baseClass::ScreenToClient(&pt);
      UINT uFlags = 0;
      HTREEITEM hItem = baseClass::HitTest(pt, &uFlags);
      // Send click event
      if( hItem != NULL ) {
         IProperty* prop = reinterpret_cast<IProperty*>(TBase::GetItemData(hItem));
         ATLASSERT(prop);
         ...
         case PROPKIND_SIMPLE:
            ...
            break;
         default:
            if(baseClass::GetSelectedItem() != hItem ) baseClass::SelectItem(hItem); // When clicking on the right
            if( (uFlags & TVHT_ONITEMRIGHT) != 0 ) bClick = true;
            break;
         }
         if( bClick ) {
            // Ask owner first
            NMPROPERTYITEM nmh = { baseClass::m_hWnd, baseClass::GetDlgCtrlID(), PIN_CLICK, prop };
            if( ::SendMessage(baseClass::GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh) == 0 ) {
               // Send Click action
   ...
   LRESULT OnDblClick(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& bHandled)
   {
      ...
      if( hItem != NULL ) {
         ...
         ATLASSERT(prop);
         // Ask owner first
         NMPROPERTYITEM nmh = { baseClass::m_hWnd, baseClass::GetDlgCtrlID(), PIN_DBLCLICK, prop };
         if( ::SendMessage(baseClass::GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh) == 0 ) {
   ...
   LRESULT OnSelChanged(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& /*bHandled*/)
   {
      ...
      // Let owner know
      NMPROPERTYITEM nmh = { baseClass::m_hWnd, baseClass::GetDlgCtrlID(), PIN_SELCHANGED, prop };
      ::SendMessage(baseClass::GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh);
      return 0;
   }
   ...
   LRESULT OnExpanding(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled)
   {
      ...
      NMPROPERTYITEM nmh = { baseClass::m_hWnd, baseClass::GetDlgCtrlID(), nMsg, prop };
      if( ::SendMessage(baseClass::GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh) != 0 ) return TRUE;
   ...
   void DrawTreeItem(CDCHandle dc, HTREEITEM hItem, UINT iState)
   {
      ...
      if( (baseClass::GetStyle() & WS_DISABLED) != 0 ) {
