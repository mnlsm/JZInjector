...
{
public:
    typedef CWindowImplBaseT<CEdit, CControlWinTraits> baseClass;
   DECLARE_WND_SUPERCLASS2(NULL, baseClass, CEdit::GetWndClassName())
...
public:
   ...
   LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
   {
      LRESULT lRes = baseClass::DefWindowProc(uMsg, wParam, lParam);
      RECT rcClient = { 0 };
      baseClass::GetClientRect(&rcClient);
      int cy = rcClient.bottom - rcClient.top;
      // Setup EDIT control
      baseClass::SetFont( CWindow(baseClass::GetParent()).GetFont() );
      baseClass::ModifyStyle(WS_BORDER, ES_LEFT);
      baseClass::SendMessage(EM_SETMARGINS, EC_LEFTMARGIN|EC_RIGHTMARGIN, MAKELONG(PROP_TEXT_INDENT, ::GetSystemMetrics(SM_CXVSCROLL)));
      // Create button
      RECT rcButton = { rcClient.right - cy, rcClient.top - 1, rcClient.right, rcClient.bottom };
      m_wndButton.Create(this, 1, baseClass::m_hWnd, &rcButton, NULL, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | BS_PUSHBUTTON | BS_OWNERDRAW);
      ATLASSERT(m_wndButton.IsWindow());
      m_wndButton.SetFont(baseClass::GetFont());
   ...
   LRESULT OnKillFocus(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
   {
      if( (HWND) wParam != m_wndButton ) ::SendMessage(baseClass::GetParent(), WM_USER_PROP_UPDATEPROPERTY, 0, (LPARAM)baseClass::m_hWnd);
      bHandled = FALSE;
      return 0;
   }
   ...
   LRESULT OnKeyDown(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
   {
      ...
      case VK_ESCAPE:
         ...
         ::PostMessage(baseClass::GetParent(), wParam == VK_RETURN ? WM_USER_PROP_UPDATEPROPERTY : WM_USER_PROP_CANCELPROPERTY, 0, (LPARAM)baseClass::m_hWnd);
         ::SetFocus(baseClass::GetParent());
         break;
      case VK_TAB:
      case VK_UP:
      case VK_DOWN:
         return ::PostMessage(baseClass::GetParent(), WM_USER_PROP_NAVIGATE, LOWORD(wParam), 0);
      case VK_LEFT:
         int lLow, lHigh;
         baseClass::SendMessage(EM_GETSEL, (WPARAM) &lLow, (LPARAM) &lHigh);
         if( lLow != lHigh || lLow != 0 ) break;
         return ::PostMessage(baseClass::GetParent(), WM_USER_PROP_NAVIGATE, LOWORD(wParam), 0);
      case VK_RIGHT:
          baseClass::SendMessage(EM_GETSEL, (WPARAM) &lLow, (LPARAM) &lHigh);
         if( lLow != lHigh || lLow != baseClass::GetWindowTextLength() ) break;
         return ::PostMessage(baseClass::GetParent(), WM_USER_PROP_NAVIGATE, LOWORD(wParam), 0);
      }
      bHandled = FALSE;
      return 0;
   ...
   LRESULT OnButtonKeyDown(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
   {
      ...
      case VK_SPACE:
         ...
         return 0;
      case VK_ESCAPE:
         ::PostMessage(baseClass::GetParent(), WM_USER_PROP_UPDATEPROPERTY, 0, (LPARAM)baseClass::m_hWnd);
         return 0;
      }
      bHandled = FALSE;
      return 0;
   ...
   LRESULT OnGetDlgCode(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
   {
      return baseClass::DefWindowProc(uMsg, wParam, lParam) | DLGC_WANTALLKEYS;
   }
};
...
public:
   ...
   LRESULT OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
   {
      ...
      m_wndList.ModifyStyle(CBS_SORT, 0);
